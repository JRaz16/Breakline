...
var wall_regrab_cooldown := 0.5
var wall_regrab_timer := 0.0

var WALL_JUMP_GRACE_TIME := 0.3
...
var footstep_interval := 0.42
var footstep_fade_speed := 8.0
var footstep_target_volume := -80.0  # muted when not moving
var footstep_active_volume := -8.0    # normal walking volume
var slide_fade_speed := 10.0
var slide_target_volume := -80.0
var slide_active_volume := -6.0
...
@onready var stand_check := $Neck/StandCheckRay
@onready var anim_3rd = $"3rdPersonAnimation"
@onready var anim_1st = $Neck/player_arms/FirstPersonAnimation
@onready var dash_audio := $"Neck/Camera3D/Dash Sound"
@onready var wall_move_audio := $Neck/Camera3D/WallFootsteps
...
func _ready():
	...
	# Command Design Pattern
	commands["dash"] = DashCommand.new()
	commands["jump"] = JumpCommand.new()
...
func _unhandled_input(event: InputEvent) -> void: 
	...
	if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		if event is InputEventMouseMotion:
			if not is_quick_turning and not is_rolling:
...
func _physics_process(delta: float) -> void:
	...
	if wall_regrab_timer > 0:
		wall_regrab_timer -= delta
	# Gravity
	if not is_on_floor():
		...
		if fall_distance >= FALL_DISTANCE_THRESHOLD:
			$"Neck/Camera3D/Landing Sound".play()
	...
	if not is_on_floor():
		...
		if not is_wall_climbing:
			if wall_regrab_timer <= 0 and is_wall_usable(current_wall) and Input.is_action_pressed("move_forward") and current_wall != last_wall:
				var normal = wall_ray_forward.get_collision_normal()

				# only vertical walls
				if abs(normal.y) < 0.2 and is_wall_usable(current_wall):
		...
		if is_wall_climbing:
			...
			if wall_climb_timer <= 0 or !wall_ray_forward.is_colliding():
				is_wall_climbing = false
				wall_move_audio.volume_db = lerp(wall_move_audio.volume_db, -80.0, 10 * delta)

				if wall_move_audio.volume_db <= -70:
					wall_move_audio.stop()
				velocity.y = LEDGE_CLIMB_SPEED
				just_wall_climbed = true
				wall_regrab_timer = wall_regrab_cooldown
		...
		# WALL RUN TIMER
		if not is_wall_climbing and not wall_ray_forward.is_colliding() and not is_wall_running and velocity.y < 1:
			...
			# valid wall check
			if not is_wall_climbing and is_wall_usable(current_wall) and current_wall != last_wall and current_wall != last_wall_run_wall and abs(normal.y) < 0.2 and Input.is_action_pressed("move_forward"):
	...
	if Input.is_action_just_pressed("jump"):
		# Command Design Pattern
		commands["jump"].execute(self)
			
		if is_wall_climbing or wall_jump_grace_timer > 0:
	...
	if is_on_floor() and not is_sliding and not is_rolling:
		...
		var target_strafe_speed = input_dir.x * (CROUCH_SPEED if is_crouching else STRAFE_SPEED)
		...
		if input_dir.y < 0:
			...
			current_forward_speed = input_dir.y * (CROUCH_SPEED if is_crouching else STRAFE_SPEED)
	...
	if is_sliding or is_rolling:
		...
		if is_rolling:
			...
			# Settings
			var SLIDE_SLOPE_ANGLE := 15.0
			var SLOPE_ACCEL := 20.0

			# Current horizontal velocity
			var horizontal = Vector3(velocity.x, 0, velocity.z)

			# On steep slope
			if slope_angle > SLIDE_SLOPE_ANGLE and is_on_floor():

				# Downhill direction
				var slope_direction = Vector3.DOWN.slide(floor_normal).normalized()

				# Accelerate downhill
				horizontal += slope_direction * SLOPE_ACCEL * delta

				# Much weaker friction on slopes
				horizontal = horizontal.move_toward(Vector3.ZERO, (SLIDE_FRICTION * 0.25) * delta)

			else:
				# Normal timed slide on flat ground
				horizontal = horizontal.move_toward(Vector3.ZERO, SLIDE_FRICTION * delta)

				slide_timer -= delta

				if slide_timer <= 0 or horizontal.length() < 1:
					is_sliding = false
	...
	if is_rolling:
		...
		roll_camera_offset = roll_angle
		
		if roll_progress >= 1.0:
			is_rolling = false
			roll_progress = 1.0
			#roll_camera_offset = 0.0
			
	else:
		roll_camera_offset = lerp(roll_camera_offset, 0.0, 10 * delta)
	...
	var target_fov = lerp(75.0, 85.0, speed_ratio)
	
	# Smooth FOV change
	camera.fov = lerp(camera.fov, target_fov, 8.0 * delta)
	
	camera.rotation.x = pitch + roll_camera_offset
	
	update_view_models(delta)
	
	# Running Sounds
	var footsteps := $Neck/Camera3D/Footsteps

	if should_play_footsteps():
		# turn sound ON smoothly
		footsteps.volume_db = lerp(
			footsteps.volume_db,
			footstep_active_volume,
			footstep_fade_speed * delta
		)
		...
		# speed up/slow down playback based on movement
		var horizontal_speed = Vector3(velocity.x, 0, velocity.z).length()
		speed_ratio = clamp(horizontal_speed / SPEED, 0.6, 1.4)

		footsteps.pitch_scale = speed_ratio

	else:
		# fade OUT smoothly
		footsteps.volume_db = lerp(
			footsteps.volume_db,
			footstep_target_volume,
			footstep_fade_speed * delta
		)

		# optional: stop when fully silent
		if footsteps.volume_db <= -70:
			footsteps.stop()
	
	# Slide Sounds
	var slide_audio := $Neck/Camera3D/SlideSound

	if is_sliding:
		# fade IN
		slide_audio.volume_db = lerp(
			slide_audio.volume_db,
			slide_active_volume,
			slide_fade_speed * delta
		)
		...
		# optional pitch variation based on speed
		var horizontal_speed = Vector3(velocity.x, 0, velocity.z).length()
		speed_ratio = clamp(horizontal_speed / SLIDE_MIN_SPEED, 0.8, 1.2)
		slide_audio.pitch_scale = speed_ratio
		...
		if slide_audio.volume_db <= -70:
			slide_audio.stop()
	
	# Wall Sounds
	var wall_movement := is_wall_running or is_wall_climbing
	var wall_audio := wall_move_audio
	...
	if wall_movement:
		...
		wall_audio.volume_db = lerp(
			wall_audio.volume_db,
			target_volume,
			10 * delta
		)

		var speed_factor = velocity.length() / WALL_RUN_SPEED
		wall_audio.pitch_scale = clamp(speed_factor, 0.9, 1.15)

	else:
		# ALWAYS force fade-out when not walling
		wall_audio.volume_db = lerp(
			wall_audio.volume_db,
			-80.0,
			12 * delta
		...
		# HARD STOP once silent
		if wall_audio.volume_db <= -70.0:
			wall_audio.stop()
	
	move_and_slide()
	
	
func is_wall_usable(wall: Object) -> bool:
	if wall == null:
		return false

	if wall.is_in_group("no_climb"):
		return false
...
func update_animations():
	...
	if anim_3rd.current_animation != state:
		anim_3rd.play(state)

	if anim_1st.current_animation != state:
		anim_1st.play(state)

func update_view_models(delta):
	# 3rd person follows yaw only
	var target_yaw = neck.rotation.y
	$ThirdPersonModel.rotation.y = lerp_angle(
		$ThirdPersonModel.rotation.y,
		target_yaw,
		12 * delta
	)

	# 1st person arms
	# 1st person arms follow camera EXACTLY
	var arms = $Neck/player_arms
	
	# yaw (left/right)
	arms.rotation.y = 0

	# crouch / slide lowering
	if is_crouching or is_sliding:
		arms.position.y = -1
	else:
		arms.position.y = -0.5

func should_play_footsteps() -> bool:
	if not is_on_floor():
		return false
	if is_sliding or is_rolling:
		return false
...
# The player does not directly control scene changes,
# UI, or game state management.
# Instead, it broadcasts gameplay events through the
# EventBus so other systems can react independently.

func _on_goal_ring_body_entered(body: Node3D) -> void:
	if body == self:
		var next_path = get_next_level_path()
		if next_path == "":
			EventManager.level_completed.emit("res://Scenes/end_screen.tscn")
			return
		if ResourceLoader.exists(next_path):
			EventManager.level_completed.emit(next_path)
		else:
			EventManager.level_completed.emit("res://Scenes/end_screen.tscn")
